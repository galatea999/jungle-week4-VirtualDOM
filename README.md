...
두 VNode를 재귀적으로 비교해 변경 목록(`patches`)을 수집하는 **diff 알고리즘**과, 이 목록을 받아 실제 DOM에 순서대로 반영하는 **patch 함수**를 구현했습니다.

핵심 설계 결정:
- 5가지 케이스(create / remove / replace / text / props)로 변경 상황을 분류해 각각 다른 방식으로 처리
- 자식 노드 비교 시 **key-prop 방식**을 적용해 불필요한 replace를 방지 (key 없는 노드는 인덱스 순서로 fallback)
- MutationObserver를 활용해 Diff가 예측한 변경 계획과 브라우저가 실제로 감지한 DOM 변화를 나란히 검증
